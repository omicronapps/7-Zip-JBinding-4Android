...
public abstract class ExtractFileAbstractTest extends JUnitNativeTestBase {
    ...
    protected class ExtractionInArchiveTestHelper {
        ...
        public IInArchive openArchiveFileWithSevenZip(int fileIndex, int compressionIndex,
            ...
            if ((JUnitTestBase.getFileNameFromAssets(archiveFilename + (usingZippedTestArchive() ? ".zip" : "")) == null || !new File(JUnitTestBase.getFileNameFromAssets(archiveFilename + (usingZippedTestArchive() ? ".zip" : ""))).exists()) && extention.contains("part1.rar")) {
                archiveFilename = archiveFilename.replace("part1.rar", "part01.rar");
                if (JUnitTestBase.getFileNameFromAssets(archiveFilename + (usingZippedTestArchive() ? ".zip" : "")) == null || !new File(JUnitTestBase.getFileNameFromAssets(archiveFilename + (usingZippedTestArchive() ? ".zip" : ""))).exists()) {
        ...
        private IInArchive openArchiveFileWithSevenZip(String archiveFilename, boolean autodetectFormat)
            ...
            try {
                if (usingZippedTestArchive()) {
                    ZipFile zipFile = new ZipFile(new File(JUnitTestBase.getFileNameFromAssets(archiveFilename + ".zip")));
                    randomAccessFileInStream = new ZipInStream(zipFile, zipFile.entries().nextElement());
                } else {
                    randomAccessFile = new RandomAccessFile(JUnitTestBase.getFileNameFromAssets(archiveFilename), "r");
    ...
    public static class VolumeArchiveOpenCallback implements IArchiveOpenCallback, IArchiveOpenVolumeCallback,
        ...
        public IInStream getStream(String filename) {
            try {
				String fullfilename = currentDir + File.separatorChar + new File(filename).getName();
				...
				randomAccessFile = new RandomAccessFile(JUnitTestBase.getFileNameFromAssets(fullfilename), "r");
